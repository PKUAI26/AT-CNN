...
from dataset import create_test_dataset, create_train_dataset, \
    ...
    create_style_test_dataset, create_brighness_test_dataset, create_patch_test_dataset, create_lowpass_test_dataset, \
    create_highpass_test_dataset, create_randpass_test_dataset
...
def test_model_genera(net, dl, dl_teacher):
    ...
    #print(acc1s.shape)
    #print(acc1s, acc3s, aacc1s, aacc3s)
    print('accuracy top-1: {:.2f}  top-3: {:.2f}'.format(np.mean(acc1s), acc3s.mean()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', type=float, default=None, help='radial filerting')
    parser.add_argument('-rh', type=float, default=None, help='High pass filerting')
    parser.add_argument('-rp', type=float, default=None, help='Random pass filerting')
    ...
    if args.r is not None:
        print('Creating low pass filertered data')
        dl = create_lowpass_test_dataset(32, './', args.r)
    if args.rh is not None:
        print('Creating high pass filertered data')
        dl = create_highpass_test_dataset(32, './', args.rh)
    if args.rp is not None:
        print('Creating random pass filertered data')
        dl = create_randpass_test_dataset(32, './', args.rp)
